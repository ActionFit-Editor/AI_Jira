...
This is a deprecated compatibility path. For new planning, route to `$plan-create`. Continue here only when the user explicitly requires a Jira issue as the planning store, requests title-only Jira intake, or selects an existing Jira needs-plan issue whose managed-description and baseline contracts must be preserved.
...
Inside the existing `## Validation Plan`, distinguish required validation, conditional escalation triggers, and intentionally excluded expensive validation. Classify deferred functional checks as automatic when AI can efficiently execute them through repository-owned tests, Unity Editor, an approved Player build, or an approved connected device; classify subjective, difficult, or inefficient checks as manual. When both are required, automatic comes first. Record the selected Unity evidence level when applicable. Generic mobile QA does not authorize a Player build, and signing, upload, distribution, deployment, credentials, and runner-secret work retains separate explicit approval. Do not add a new top-level heading or schema field for this classification.
