...
- Its `Validation Plan` distinguishes required validation, conditional escalation triggers, and intentionally excluded expensive validation. A Player build absent from that approved plan or a signing, upload, distribution, deployment, credential, or runner-secret operation makes the candidate approval-required.
...
4. After approval, re-read and require the same todo status and captured `updated` value. If either differs, regenerate and reapprove without transitioning. After a match, run `transition <ISSUE-KEY> --to progress --purpose planning --json` to seal the pre-refinement source requirements. Do not create a branch or worktree for plan-only work.
5. Compare every sealed source requirement with the approved draft, create the plan-coverage JSON required by `references/completion-baseline-gate.md`, and call `update-description --mode replace-plan --expected-updated ... --coverage-file ...`. Removed, deferred, or out-of-scope work requires separate explicit replanning approval and may never be inferred from partial-PR approval.
6. For plan only, run `transition <ISSUE-KEY> --to todo --purpose planning`, verify, and stop. For plan update and auto-start, continue immediately to the sealed implementation start and do not end the invocation in `progress`.
...
2. For every implementation, run `python3 .agents/skills/jira-auto-start/scripts/ai_jira_write_cli.py start <ISSUE-KEY> --branch <ACTUAL-BRANCH> --json` after the visible Jira announcement and Jira-key-verified branch/worktree plan exist. Preserve its session ID, baseline digest, and requirement IDs. Never use a generic progress transition; a refined issue converts its valid planning lock through the same start command. Legacy progress without a baseline must finalize incomplete and restart from todo.
...
Approval waiting stays in todo. If the same todo status, captured `updated`, or canonical draft is lost, regenerate the Korean approval view before taking a transient planning lock.
