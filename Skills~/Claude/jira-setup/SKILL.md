...
python3 .claude/skills/jira-setup/scripts/ai_jira_cli.py \
  list --state todo --format json
```

Keep `dry_run` and every write gate disabled. Report the config path, safe status/gate summary, authenticated read success or categorized failure, and any missing user action without reproducing issue content beyond what the user requested.
