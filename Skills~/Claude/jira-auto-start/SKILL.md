...
10. For needs-plan, re-read todo, transition to progress as a planning lock, verify and capture `updated`, then prepare and retain the canonical Jira Plan mixed-language draft without creating a worktree. Read `references/korean-approval-preview.md`, show its complete Korean approval view, and ask for plan only or plan update and auto-start approval. Explain that approval writes the corresponding pre-preview canonical draft. Update only with `update_description.py --mode replace-plan --expected-updated ...` using that draft. Return to todo for plan only or keep progress and continue for approved implementation. On failure, attempt verified todo rollback; never expire or steal an interrupted lock. If canonical state is unavailable or uncertain, regenerate both representations and obtain approval again.
