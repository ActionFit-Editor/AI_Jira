...
- A requested revision invalidates prior approval: update the canonical draft first, regenerate the complete Korean approval view, and request approval again. If interruption or context loss makes the canonical draft unavailable or uncertain, regenerate both representations and reapprove before any Jira write. New-issue and existing needs-plan discussion stay in `todo`. After approval, recheck the todo snapshot, acquire a transient progress lock, capture post-transition `updated`, and call only the project `replace-plan` operation. Plan-only returns to todo; plan-and-implementation continues immediately. Update failure attempts todo rollback. Locks never expire or get stolen automatically.
